...
def main():
    ...
    als = ALS(implicitPrefs=True, maxIter=int(args.maxIter), regParam=float(args.regParam), alpha=int(args.alpha), userCol="idpersona", itemCol="securityid",ratingCol="q", coldStartStrategy="drop")
    ...
    df = userRecs.select(userRecs.idpersona,explode(userRecs.recommendations).alias("securityid_rating"))
