...
class ag_recommendations_securityid(Model):
    ...
    def train(self):
        ...
        processing_job_script_s3_key = f"batch-inferences/{self.config['model_name']}/{self.config['model_id']}/processing_jobs/ag_train_spark_als.py"
        processing_job_script_s3_uri = f"s3://{self.config['bucket_models']}/{processing_job_script_s3_key}"
        s3 = boto3.resource('s3')
        model_bucket = s3.Bucket(self.config['bucket_models']).upload_file('processing_jobs/ag_train_spark_als.py', processing_job_script_s3_key)
    ...
    def tuner(self):
        ...
        # Upload python PySparkProcessor script to s3
        processing_job_script_s3_key = f"batch-inferences/{self.config['model_name']}/{self.config['model_id']}/processing_jobs/ag_tuner_spark_als.py"
        processing_job_script_s3_uri = f"s3://{self.config['bucket_models']}/{processing_job_script_s3_key}"
        s3 = boto3.resource('s3')
        model_bucket = s3.Bucket(self.config['bucket_models']).upload_file('processing_jobs/ag_tuner_spark_als.py', processing_job_script_s3_key)
    ...
    # TODO: declarar funcion send_instruments si hiciera falta
    ...
    def post_processing(self):
        ...
        df = df[['idpersona','securityid','grupo','rating']]
        #logging.info(df.head())
        logging.info(
            "#############GENERAL RECOMMENDATION #############")

        recs_general = pd.DataFrame()
        recs_general = recs_general.append(pd.DataFrame(df[df['grupo'] == 'CEDEARS'].securityid.value_counts(ascending = False)).head(3))
        recs_general = recs_general.append(pd.DataFrame(df[df['grupo'] == 'Acciones'].securityid.value_counts(ascending = False)).head(3))
        recs_general = recs_general.append(pd.DataFrame(df[df['grupo'] == 'Renta Fija'].securityid.value_counts(ascending = False)).head(4))
        # logging.info(recs_general.head())
        recs_general = recs_general.sort_values(by = 'securityid', ascending = False).reset_index()
        recs_general.rename(columns = {'index': 'securityid', 'securityid': 'securityid'}, inplace = True)
        general_recommendation = pd.DataFrame(recs_general.iloc[:,0]).to_dict(orient = 'records') # recomendacion general
        logging.info(general_recommendation)
        ...
        # TODO: put_obj_in_bucket(general_recommendation)
