...
class ag_recommendations(Model):
    ...
    def data_preparation(self, df):
        ...
        logging.info(f'Train obj: {train_obj}')
        logging.info(f'encoding: {encoding}')
        logging.info(f'train_key: {self.train_key}')
        logging.info(f"train_key: {self.config['feature_engineering']['dataset_format']}")

        #logging.info(f'Test obj: {test_obj}')
    ...
    def cut_df(self, df):
        ...
        user_group = df.groupby('idpersona').q.count().sort_values()
        
        users_varias_inv = user_group.index[df.groupby(
            'idpersona').q.count().sort_values() > 1]

        # Ultima inversion para cada usuario
        user_group_2 = df.loc[df['idpersona'].isin(
            users_varias_inv)].reset_index().groupby('idpersona').nth(-1)
